...
# Navigation
# ─────────────────────────────────────────────
NAV_OPTIONS = ["📂 פורטפוליו נוכחי", "⚙️ הגדרות יעד", "📈 תוצאות", "⚖️ השוואת מסלולים", "🕓 היסטוריה"]
...
if active_tab == "📈 תוצאות":
    ...
    if res is None:
        st.info("עבור לטאב **הגדרות יעד** ולחץ **חשב חלופות מיטביות**.")
    else:
        ...
        if not recs:
            ...
            # ── Helper: compute required changes vs current portfolio ──
            def _count_changes(rrow: Dict) -> str:
                """
                Compare proposed tracks/managers to current portfolio holdings.
                Returns an HTML string: e.g. "🔄 שינוי מסלול ×2 | 🔁 ניוד ×1"
                """
                ...
                nidud  = 0  # full transfer – new manager
                shinui = 0  # track change  – same manager
            ...
            # ── Build tracks display string (pct% מסלול | …) ──
            ...
            show_changes   = has_holdings  # show changes column when we have holdings
            ...
            def _pct_fmt(v, key):
                try:
                    return f"{float(v):.1f}%" if "%" in key else f"{float(v):.2f}"
                except: return "—"
            ...
            def _make_data_row(label, rrow_or_none, row_style="", label_style=""):
                """Build a <tr> for an alternative or special row (targets / baseline)."""
                cells = f"<td style='font-weight:800;{label_style}'>{_esc(label)}</td>"

                # tracks cell – only for alternative rows
                ...
                for col_lbl, col_key in zip(num_cols, row_keys):
                    if rrow_or_none is None:
                        ...
                        val = rrow_or_none.get(col_key, "—") if "%" in col_key or col_key in row_keys else "—"
                        # for baseline / targets rows passed as plain dict
            ...
            # Special row: current state (if available)
