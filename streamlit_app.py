...
/* ── Premium Header ── */
.app-header {
  background: linear-gradient(135deg, #0f1e3d 0%, #1a3560 60%, #1e4080 100%);
  border-radius: 18px;
...
.header-stat .hval { font-size: 20px; font-weight: 800; color: #60a5fa; }
.header-stat .hlbl { font-size: 11px; color: #93b4e0; letter-spacing: 0.5px; }
...
.profile-hint { font-size: 12px; color: #64748b; margin-bottom: 10px; }
@media (prefers-color-scheme: dark) {
  .app-header { background: linear-gradient(135deg, #0a1628 0%, #0f2040 60%, #112350 100%); }
  .profile-section { background: #0f172a; border-color: #1e3a8a; }
  .profile-title { color: #93c5fd; }
}
...
.vote-stats-card {
  border: 1px solid #e2e8f0; border-radius: 14px; padding: 12px 14px; background: #ffffff; margin-bottom: 10px;
}
.vote-stats-title { font-weight: 800; color: #0f172a; margin-bottom: 6px; }
.vote-stats-sub { font-size: 12px; color: #64748b; }
...
@media (prefers-color-scheme: dark) {
  ...
  .stRadio[role="radiogroup"] label:has(input:checked) {
    background: linear-gradient(135deg, rgba(30,58,138,0.35) 0%, rgba(37,99,235,0.22) 100%);
    border-color: #60a5fa;
  }
  .stRadio[role="radiogroup"] label:has(input:checked) p {
    color: #bfdbfe;
...
.compact-panel-title {
  font-size: 18px; font-weight: 800; color: #0f172a; margin: 0 0 4px; text-align: right;
}
.compact-panel-sub {
  font-size: 12px; color: #64748b; margin: 0 0 10px; text-align: right;
}
.target-grid-header, .target-grid-row {
  display: grid;
  grid-template-columns: minmax(120px, 1.15fr) 0.95fr 1.2fr 0.9fr 0.9fr;
  gap: 8px;
  align-items: center;
}
.target-grid-header {
...
.target-grid-header div {
  font-size: 11px; font-weight: 800; color: #475569; text-align: right;
}
.target-grid-row {
  border: 1px solid #e2e8f0;
  border-radius: 14px;
...
.target-label {
  ...
  font-weight: 800; color: #0f172a; min-height: 36px; text-align: right;
}
.compact-top-card {
  border: 1px solid #dbeafe;
  border-radius: 14px;
...
.compact-top-card h4 {
  margin: 0 0 10px; font-size: 14px; font-weight: 800; color: #1e3a8a; text-align: right;
}
.results-anchor {
...
@media (max-width: 1000px) {
  ...
  .target-grid-header { display: none; }
  .target-grid-row { padding: 10px; }
}
@media (prefers-color-scheme: dark) {
  .compact-panel { background: linear-gradient(180deg, #0f172a 0%, #111827 100%); border-color: #334155; }
  .compact-panel-title { color: #f8fafc; }
  .compact-panel-sub { color: #94a3b8; }
  .target-grid-header div { color: #cbd5e1; }
  .target-grid-row { background: #0b1220; border-color: #334155; }
  .target-label { color: #f8fafc; }
  .compact-top-card { background: #0f172a; border-color: #1d4ed8; }
  .compact-top-card h4 { color: #93c5fd; }
}

/* ══ COMPACT RESULT CARDS (Bloomberg/Betterment style) ══ */
.rc {
  border: 1px solid #e2e8f0; border-radius: 14px;
...
.rc.rc-primary { border: 2px solid #2563eb; box-shadow: 0 4px 20px rgba(37,99,235,0.13); }
.rc.rc-service { border: 2px solid #10b981; box-shadow: 0 4px 20px rgba(16,185,129,0.10); }
...
.rc.rc-primary .rc-head { background: linear-gradient(135deg, #eff6ff 0%, #dbeafe 100%); }
.rc.rc-service .rc-head { background: linear-gradient(135deg, #f0fdf4 0%, #dcfce7 100%); }
.rc-title { font-size: 13px; font-weight: 800; color: #0f1e3d; }
.rc-badge { font-size: 11px; font-weight: 700; padding: 2px 9px; border-radius: 999px; white-space: nowrap; }
.rc-badge-blue { background: #dbeafe; color: #1e40af; }
.rc-badge-green { background: #dcfce7; color: #065f46; }
.rc-badge-amber { background: #fef3c7; color: #92400e; }
.rc-badge-red   { background: #fee2e2; color: #991b1b; }

/* Fund allocation bars */
.rc-funds { padding: 8px 14px 6px; }
.rc-fund-row {
  display: flex; align-items: center; gap: 8px;
  padding: 3px 0; font-size: 12px; color: #1e293b;
}
.rc-fund-pct { min-width: 36px; font-weight: 800; font-size: 13px; color: #0f1e3d; }
.rc-fund-bar-wrap { flex: 1; height: 6px; background: #e9eef8; border-radius: 99px; overflow: hidden; }
.rc-fund-bar { height: 100%; border-radius: 99px; }
.rc-fund-name { font-size: 11px; color: #475569; max-width: 130px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
...
.rc-kpi {
  ...
  background: #f8fafc; border: 1px solid #e9eef8;
  border-radius: 8px; padding: 4px 9px;
}
.rc-kpi-label { font-size: 10px; color: #64748b; font-weight: 600; }
.rc-kpi-val   { font-size: 13px; font-weight: 800; color: #0f1e3d; }
.rc-kpi-delta { font-size: 11px; font-weight: 700; }
.rc-kpi-delta.up   { color: #16a34a; }
.rc-kpi-delta.down { color: #dc2626; }
.rc-kpi-delta.flat { color: #94a3b8; }

/* Delta table (current vs proposed) */
.rc-delta {
  padding: 6px 14px 8px;
  border-top: 1px solid #f1f5f9;
}
.rc-delta-title { font-size: 10px; font-weight: 700; color: #64748b; letter-spacing: 0.5px; text-transform: uppercase; margin-bottom: 5px; }
.rc-delta-grid {
...
.rc-delta-item-label { font-size: 10px; color: #64748b; font-weight: 600; }
.rc-delta-item-vals  { display: flex; align-items: center; gap: 4px; margin-top: 2px; }
.rc-delta-item-cur   { font-size: 11px; color: #94a3b8; text-decoration: line-through; }
.rc-delta-item-new   { font-size: 13px; font-weight: 800; color: #0f1e3d; }
.rc-delta-item-arrow { font-size: 11px; font-weight: 700; }
.rc-delta-item-arrow.up   { color: #16a34a; }
.rc-delta-item-arrow.down { color: #dc2626; }
.rc-delta-item-arrow.flat { color: #94a3b8; }
...
.rc-ai-label { font-size: 10px; font-weight: 700; color: #6366f1; letter-spacing: 0.4px; margin-bottom: 3px; }
.rc-ai-text  { font-size: 12px; color: #334155; line-height: 1.55; }
.rc-ai-spinner { font-size: 11px; color: #94a3b8; font-style: italic; }
...
.res-summary-item { display: flex; flex-direction: column; align-items: center; min-width: 80px; }
.res-summary-item .rsval { font-size: 18px; font-weight: 800; color: #1e3a8a; }
.res-summary-item .rslbl { font-size: 10px; color: #64748b; letter-spacing: 0.3px; }

/* Compact comparison table */
.compare-tbl { width: 100%; border-collapse: collapse; font-size: 12px; direction: rtl; margin-top: 8px; }
.compare-tbl th { background: #0f1e3d; color: #e2e8f0; padding: 8px 10px; text-align: center; white-space: nowrap; font-weight: 700; }
.compare-tbl th:first-child { text-align: right; min-width: 120px; }
.compare-tbl td { padding: 7px 10px; border-bottom: 1px solid #f1f5f9; text-align: center; white-space: nowrap; }
.compare-tbl td:first-child { text-align: right; font-weight: 700; color: #1e3a8a; max-width: 180px; overflow: hidden; text-overflow: ellipsis; }
.compare-tbl tr:last-child td { border-bottom: none; }
.compare-tbl tr:hover td { background: #f8faff; }

@media (prefers-color-scheme: dark) {
  .rc { background: #111827; border-color: #1e3a5f; }
  .rc-head { background: linear-gradient(135deg, #0f172a, #0d1d3a); border-color: #1e3a5f; }
  .rc.rc-primary .rc-head { background: linear-gradient(135deg, #0f1f3d, #152347); }
  .rc.rc-service .rc-head { background: linear-gradient(135deg, #052e16, #064e2f); }
  .rc-title { color: #e2e8f0; }
  .rc-fund-row { color: #cbd5e1; }
  .rc-fund-pct { color: #f1f5f9; }
  .rc-fund-bar-wrap { background: #1e293b; }
  .rc-fund-name { color: #94a3b8; }
  .rc-kpis, .rc-delta, .rc-ai { border-color: #1e293b; }
  .rc-kpi { background: #0f172a; border-color: #1e293b; }
  .rc-kpi-label { color: #94a3b8; }
  .rc-kpi-val { color: #f1f5f9; }
  .rc-delta-item { background: #0f172a; border-color: #1e293b; }
  .rc-delta-item-label { color: #94a3b8; }
  .rc-delta-item-new { color: #f1f5f9; }
  .rc-ai { background: linear-gradient(135deg, #0f172a, #0d1625); }
  .rc-ai-text { color: #cbd5e1; }
  .compare-tbl th { background: #1e3a8a; }
  .compare-tbl td { border-color: #1e293b; color: #cbd5e1; }
  .compare-tbl td:first-child { color: #93c5fd; }
  .compare-tbl tr:hover td { background: #0f172a; }
  .res-summary { background: #0f172a; border-color: #1e3a5f; }
  .res-summary-item .rsval { color: #60a5fa; }
}
</style>
...
active_tab = st.session_state.get("active_tab", "⚙️ הגדרות יעד")
...
def _has_missing_sharpe_value(x) -> bool:
    try:
        if x is None:
            return True
        val = float(x)
        return math.isnan(val) or abs(val) < 1e-12
    except Exception:
        return True

def _sharpe_note_text(flag: bool) -> str:
    return "לא כל המסלולים בחלופה פרסמו מדד שארפ" if flag else ""
...
# ─────────────────────────────────────────────
VOTES_SHEET_NAMES = ["votes", "FUNDS_GSHEET_ID", "גיליון1", "Sheet1"]

def _ensure_votes_header(ws):
    try:
        first_row = ws.row_values(1)
    except Exception:
        first_row = []
    required = ["timestamp","alternative","managers","tracks","n_funds","mix_policy","session_hash"]
    if not first_row:
...
def _get_votes_worksheet():
    """Return the gspread worksheet for votes, supporting both legacy and new sheet names."""
    try:
        import gspread
        ...
        for ws_name in VOTES_SHEET_NAMES:
            try:
                return _ensure_votes_header(sheet.worksheet(ws_name))
            except gspread.WorksheetNotFound:
                pass

        ws = sheet.add_worksheet(title=VOTES_SHEET_NAMES[0], rows=2000, cols=8)
        return _ensure_votes_header(ws)
    except Exception:
        return None
...
def _render_votes_stats(selected_alternative: Optional[str] = None, compact: bool = False):
    """Render voting statistics, optionally focused on a selected alternative."""
    df = _load_votes_cached()

    if df.empty:
        st.info("עדיין אין נתוני בחירות של משתמשים. היה הראשון לבחור חלופה!")
        return
    ...
    c1.metric("בחירות ב-30 יום", f"{total_30:,}")
    c2.metric("בחירות ב-7 ימים", f"{total_7:,}")
    c3.metric("סה״כ כל הזמנים", f"{total_all:,}")

    if df30.empty:
        st.caption("אין בחירות ב-30 הימים האחרונים עדיין.")
        return

    alt_counts = df30["alternative"].astype(str).value_counts().reset_index()
    alt_counts.columns = ["חלופה", "בחירות"]
    alt_counts["אחוז"] = (alt_counts["בחירות"] / total_30 * 100).round(1)

    if selected_alternative:
        alt_row = alt_counts[alt_counts["חלופה"] == str(selected_alternative)]
        if not alt_row.empty:
            rank = int(alt_row.index[0]) + 1
            row = alt_row.iloc[0]
            st.markdown(
                f"<div class='vote-stats-card'>"
                f"<div class='vote-stats-title'>📊 מה בחרו משתמשים אחרים לגבי <span style='color:#1d4ed8'>{_esc(selected_alternative)}</span></div>"
                f"<div class='vote-stats-sub'>ב-30 הימים האחרונים החלופה דורגה במקום <b>#{rank}</b>, עם <b>{int(row['בחירות'])}</b> בחירות — <b>{float(row['אחוז']):.1f}%</b> מכלל הבחירות.</div>"
                f"</div>",
                unsafe_allow_html=True,
            )

            alt_df30 = df30[df30["alternative"].astype(str) == str(selected_alternative)].copy()
            if not alt_df30.empty:
                seg1, seg2 = st.columns(2)
                with seg1:
                    st.markdown("**👥 כמה משתמשים בחרו בדיוק בחלופה הזו**")
                    st.metric("ב-30 יום", f"{len(alt_df30):,}")
                    st.metric("ב-7 ימים", f"{len(df7[df7['alternative'].astype(str) == str(selected_alternative)]):,}")
                with seg2:
                    st.markdown("**🏷️ מנהלים ומסלולים שחזרו הכי הרבה בתוך החלופה**")
                    top_mgr = pd.Series([m.strip() for cell in alt_df30.get('managers', pd.Series(dtype=str)).dropna() for m in str(cell).split('|') if m.strip()]).value_counts().head(3)
                    top_trk = pd.Series([t.strip() for cell in alt_df30.get('tracks', pd.Series(dtype=str)).dropna() for t in str(cell).split('|') if t.strip()]).value_counts().head(3)
                    if not top_mgr.empty:
                        st.caption("מנהלים מובילים")
                        for mgr, cnt in top_mgr.items():
                            st.write(f"• {mgr} — {cnt}")
                    if not top_trk.empty:
                        st.caption("מסלולים מובילים")
                        for trk, cnt in top_trk.items():
                            st.write(f"• {trk} — {cnt}")
            st.markdown("---")

    st.markdown("**🏆 פופולריות החלופות – 30 יום אחרונים**")
    max_v = int(alt_counts["בחירות"].max()) if total_30 else 1
    bar_colors = {"חלופה משוקללת": "#2563eb", "הכי מדויקת": "#16a34a", "שארפ מקסימלי": "#ea580c", "שירות מוביל": "#7c3aed"}
    rows_html = ""
    for _, r in alt_counts.iterrows():
        ...
        bar_w = max(6, int(cnt / max_v * 220))
        emphasis = "font-weight:900;color:#0f172a" if selected_alternative and alt == selected_alternative else "font-weight:700"
        rows_html += (
            f"<div style='display:flex;align-items:center;gap:10px;margin:7px 0;direction:rtl'>"
            f"<span style='min-width:138px;{emphasis};font-size:13px'>{_esc(alt)}</span>"
            f"<div style='height:18px;width:{bar_w}px;background:{color};border-radius:999px'></div>"
            f"<span style='font-size:12px;color:#475569'>{cnt} ({pct:.1f}%)</span>"
            f"</div>"
        )
    st.markdown(rows_html, unsafe_allow_html=True)

    if not compact and "tracks" in df30.columns:
        st.markdown("---")
        st.markdown("**📋 המסלולים שנבחרו הכי הרבה – 30 יום**")
        all_tracks_voted = []
        ...
        if all_tracks_voted:
            ...
            for trk, cnt in tc.items():
                track_rows += (
                    f"<div style='display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid #f1f5f9;font-size:12px;direction:rtl'>"
                    f"<span style='color:#334155'>{_esc(str(trk))}</span>"
                    f"<span style='font-weight:800;color:#1e3a8a'>{cnt}</span>"
                    f"</div>"
                )
            st.markdown(f"<div style='border:1px solid #e2e8f0;border-radius:10px;padding:10px 14px'>{track_rows}</div>", unsafe_allow_html=True)

    st.caption(f"הנתונים נקראים ישירות מ-Google Sheets ומתעדכנים כל 5 דקות. עדכון אחרון: {datetime.now().strftime('%H:%M:%S')}")


@st.dialog("סטטיסטיקת משתמשים על החלופה שנבחרה", width="large")
def _vote_stats_dialog(selected_alternative: Optional[str] = None):
    _render_votes_stats(selected_alternative=selected_alternative, compact=False)
    if st.button("סגור", key="close_vote_stats_dialog"):
        st.session_state["show_vote_stats_dialog"] = False
        st.rerun()
...
def find_best_solutions(
    ...
    for combo in combo_source:
        ...
        sharpe_missing_combo = any(_has_missing_sharpe_value(records.loc[i, "sharpe"]) for i in combo)
        if sharpe_missing_combo:
            mix_ok[:, 4] = np.nan
        ...
        for wi in range(len(mix_ok)):
            solutions.append({
                ...
                "שארפ משוקלל":    (float(mix_ok[wi, 4]) if not np.isnan(mix_ok[wi, 4]) else np.nan),
                "שירות משוקלל":   float(mix_ok[wi, 5]),
                "הערת שארפ":      _sharpe_note_text(sharpe_missing_combo),
...
def _kpi_chip_html(label: str, val: float, baseline_val: Optional[float] = None,
    ...
    try:
        ...
        val_num = np.nan
    val_str = "" if math.isnan(val_num) else fmt.format(val_num)
    delta_html = ""
    if baseline_val is not None and not math.isnan(val_num):
        diff = val_num - baseline_val
        if abs(diff) < 0.1:
            cls = "flat"; arrow = "→"
...
def _delta_grid_html(row: Dict, baseline: Dict) -> str:
    ...
    for label, cur, new, lower_better, fmt in items:
        ...
        cur_disp = "" if math.isnan(cur) else fmt.format(cur)
        new_disp = "" if math.isnan(new) else fmt.format(new)
        arrow_disp = "" if math.isnan(new) else arrow
        cells += (
            f"<div class='rc-delta-item'>"
            f"<span class='rc-delta-item-label'>{_esc(label)}</span>"
            f"<div class='rc-delta-item-vals'>"
            f"<span class='rc-delta-item-cur'>{cur_disp}</span>"
            f"<span class='rc-delta-item-arrow {arrow_cls}'>{arrow_disp}</span>"
            f"<span class='rc-delta-item-new'>{new_disp}</span>"
            f"</div></div>"
        )
...
def _render_compact_card(r: Dict, title: str, card_cls: str = "",
    ...
    sharpe_note = str(r.get("הערת שארפ", "")).strip()
    sharpe_note_html = f"<div class='score-tip' style='margin:10px 14px 0'>{_esc(sharpe_note)}</div>" if sharpe_note else ""
...
def _export_excel(top3, baseline=None):
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        display_cols = [
            ...
            "שארפ משוקלל", "שירות משוקלל", "הערת שארפ", "score",
        ]
...
def _init_state():
    ...
    st.session_state.setdefault("show_vote_stats_dialog", False)
    st.session_state.setdefault("selected_vote_label", None)
    # פיצ׳ר 1
...
st.markdown('<div class="nav-wrap"><div class="nav-shell"><div class="nav-title">ניווט מהיר</div></div></div>', unsafe_allow_html=True)
nav_cols = st.columns(len(NAV_OPTIONS), gap="small")
for i, tab_name in enumerate(NAV_OPTIONS):
    with nav_cols[i]:
        if st.button(tab_name, key=f"nav_btn_{i}", use_container_width=True, type=("primary" if st.session_state.get("active_tab") == tab_name else "secondary")):
...
if active_tab == "⚙️ הגדרות יעד":
    ...
    <div class="compact-panel">
      <div class="compact-panel-title">קביעת היעדים</div>
      <div class="compact-panel-sub">כל ההגדרות והתוצאות מרוכזות במסך אחד, כך שהמשתמש רואה יותר תוכן כבר בפתיחה.</div>
    </div>
    """, unsafe_allow_html=True)

    top_c1, top_c2 = st.columns([1.05, 1.35], gap="small")
    with top_c1:
        st.markdown('<div class="compact-top-card"><h4>הגדרות בסיס</h4></div>', unsafe_allow_html=True)
        c1, c2 = st.columns(2)
    with c1:
    ...
    with c2:
        ...
        # צעד ודירוג נשארים קבועים לחוויה מיטבית
        # step=5 (מאוזן בין דיוק למהירות), primary_rank="דיוק"

    with top_c2:
        st.markdown('<div class="compact-top-card"><h4>נעילת קרן (אופציונלי)</h4></div>', unsafe_allow_html=True)
    ...
    st.markdown("""
    <div class="compact-panel">
      <div class="compact-panel-title">יעדים ומגבלות</div>
      <div class="compact-panel-sub">טבלה קומפקטית, מיושרת לימין, עם פחות רווחים ויותר מידע מעל קו הקיפול.</div>
      <div class="target-grid-header">
        <div>פרמטר</div>
        <div>כלול בדירוג</div>
        <div>יעד (%)</div>
        <div>קשיחות</div>
    ...
    def _metric_row(key, label, default_mode, max_val=100.0):
        st.markdown('<div class="target-grid-row">', unsafe_allow_html=True)
        cols = st.columns([1.15, 0.95, 1.2, 0.9, 0.9], gap="small")
        with cols[0]:
            st.markdown(f'<div class="target-label">{label}</div>', unsafe_allow_html=True)
        with cols[1]:
            inc = st.checkbox("כלול", value=st.session_state["include"].get(key, False), key=f"inc_{key}")
        with cols[2]:
            val = st.slider(
                f"יעד עבור {label}", 0.0, max_val,
                float(st.session_state["targets"].get(key, 0.0)),
                step=0.5, key=f"tgt_{key}", label_visibility="collapsed"
            )
        with cols[3]:
            hard = st.selectbox(
                f"קשיחות עבור {label}", ["רך", "קשיח"],
                index=0 if st.session_state["constraint"].get(key, ("רך",))[0] == "רך" else 1,
                key=f"hard_{key}", label_visibility="collapsed"
            )
        with cols[4]:
            mode = st.selectbox(
                f"כיוון עבור {label}", ["בדיוק", "לפחות", "לכל היותר"],
...
def render_results_section():
    ...
    if res is None:
        st.info("בחר יעדים ולחץ **חשב חלופות מיטביות** כדי לראות כאן את התוצאות מיד מתחת לאזור ההגדרות.")
        return
    ...
    # ── summary strip ──
    summary_parts = [f"<span style='color:#1e3a8a;font-weight:800'>{ts}</span>"]
    if bl_total:
        summary_parts.append(f"פורטפוליו: <b>₪{bl_total:,.0f}</b>")
    if baseline:
        summary_parts.append("<span style='color:#10b981;font-weight:700'>✓ השוואה למצב קיים זמינה</span>")
    st.markdown(
        f"<div style='font-size:12px;color:#475569;margin-bottom:8px'>"
        f"{'&nbsp;&nbsp;|&nbsp;&nbsp;'.join(summary_parts)}"
        f"&nbsp;&nbsp;<span style='opacity:.6'>{_esc(note)}</span></div>",
        unsafe_allow_html=True
    )
    ...
    def _count_changes(rrow: Dict) -> str:
        ...
        for mgr, trk in zip(prop_managers, prop_tracks):
            if mgr not in cur_managers:
                nidud += 1
            elif trk not in cur_tracks:
                shinui += 1
        parts = []
        if shinui > 0:
            parts.append(f"🔄 שינוי מסלול ×{shinui}")
        if nidud > 0:
            parts.append(f"🔁 ניוד ×{nidud}")
        if not parts:
            return "<span style='color:#16a34a;font-weight:700'>✅ ללא שינוי</span>"
    ...
    row_keys = ['חו"ל (%)', "מניות (%)", 'מט"ח (%)', "לא־סחיר (%)", "שארפ משוקלל", "שירות משוקלל"]
    ...
    def _pct_fmt(v, key):
        try:
            ...
            if math.isnan(num):
                return ""
            return f"{num:.1f}%" if "%" in key else f"{num:.2f}"
        except:
            return ""

    def _delta_style(proposed, current, is_lower_better=False):
        try:
            diff = float(proposed) - float(current)
            if abs(diff) < 0.05:
                return ""
            good = (diff > 0 and not is_lower_better) or (diff < 0 and is_lower_better)
            return f"color:{'#16a34a' if good else '#dc2626'};font-weight:800"
        except:
            return ""

    is_lower_better_map = {
        'חו"ל (%)': False, "מניות (%)": False, 'מט"ח (%)': False,
        "לא־סחיר (%)": True, "שארפ משוקלל": False, "שירות משוקלל": False
    }

    change_th = "<th>שינויים נדרשים</th>" if show_changes else ""
    header = f"<tr><th style='min-width:110px'>חלופה</th><th>מסלולים ומשקלים</th>{''.join(f'<th>{c}</th>' for c in num_cols)}{change_th}</tr>"

    def _make_data_row(label, rrow_or_none, row_style="", label_style=""):
        cells = f"<td style='font-weight:800;{label_style}'>{_esc(label)}</td>"
        if rrow_or_none is not None and "weights_items" in rrow_or_none:
            cells += f"<td style='font-size:11px;line-height:1.6'>{_tracks_display(rrow_or_none)}</td>"
        else:
            cells += "<td>—</td>"
        for col_lbl, col_key in zip(num_cols, row_keys):
            if rrow_or_none is None:
                cells += "<td>—</td>"
            else:
                val = rrow_or_none.get(col_key, rrow_or_none.get(col_lbl, "—"))
                dstyle = ""
                if has_baseline and rrow_or_none.get("_is_alt"):
                    cur = baseline.get({
                        'חו"ל (%)': "foreign", "מניות (%)": "stocks",
                        'מט"ח (%)': "fx", "לא־סחיר (%)": "illiquid",
                        "שארפ משוקלל": "sharpe", "שירות משוקלל": "service",
                    }.get(col_key,""), None)
                    if cur is not None:
                        dstyle = _delta_style(val, cur, is_lower_better_map.get(col_key, False))
                cells += f"<td style='{dstyle}'>{_pct_fmt(val, col_key)}</td>"
        if show_changes:
            if rrow_or_none is not None and rrow_or_none.get("_is_alt"):
                cells += f"<td style='font-size:11px;line-height:1.7'>{_count_changes(rrow_or_none)}</td>"
            else:
                cells += "<td>—</td>"
        return f"<tr style='{row_style}'>{cells}</tr>"

    target_row_data = {
        'חו"ל (%)': targets_used.get("foreign", "—"),
        "מניות (%)": targets_used.get("stocks", "—"),
        'מט"ח (%)': targets_used.get("fx", "—"),
        "לא־סחיר (%)": targets_used.get("illiquid", "—"),
        "שארפ משוקלל": "—",
        "שירות משוקלל": "—",
    }

    baseline_row_data = None
    if has_baseline:
        baseline_row_data = {
            'חו"ל (%)': baseline.get("foreign", "—"),
            "מניות (%)": baseline.get("stocks", "—"),
            'מט"ח (%)': baseline.get("fx", "—"),
            "לא־סחיר (%)": baseline.get("illiquid", "—"),
            "שארפ משוקלל": baseline.get("sharpe", "—"),
            "שירות משוקלל": baseline.get("service", "—"),
        }
    ...
    body += _make_data_row(
        "🎯 יעדים",
        target_row_data,
        row_style="background:#fffbeb;border-bottom:2px solid #fde68a",
        label_style="color:#92400e"
    )
    if baseline_row_data:
        body += _make_data_row(
            "📂 מצב קיים",
            baseline_row_data,
            row_style="background:#f0fdf4;border-bottom:2px solid #bbf7d0",
            label_style="color:#065f46"
        )
    body += f"<tr><td colspan='100' style='height:4px;background:#e2e8f0;padding:0'></td></tr>"

    alt_colors = ["#eff6ff", "#ffffff", "#ffffff", "#f0fdf4"]
    alt_border = ["2px solid #bfdbfe", "none", "none", "2px solid #bbf7d0"]
    for i, rrow in enumerate(rows):
        rrow["_is_alt"] = True
        body += _make_data_row(
            rrow["חלופה"],
            rrow,
            row_style=f"background:{alt_colors[i]};border-bottom:{alt_border[i]}",
            label_style="font-weight:900;color:#0f1e3d"
        )

    change_col_width = "min-width:130px" if show_changes else ""
    st.markdown(f"""
<style>
.res-tbl {{ width:100%; border-collapse:collapse; font-size:13px; direction:rtl; }}
.res-tbl th {{
  background:#0f1e3d; color:#e2e8f0; padding:9px 12px;
  text-align:center; white-space:nowrap; font-weight:700; font-size:12px;
}}
.res-tbl th:first-child {{ text-align:right; min-width:110px; }}
.res-tbl th:nth-child(2) {{ text-align:right; min-width:200px; }}
.res-tbl td {{ padding:9px 12px; border-bottom:1px solid #f1f5f9; text-align:center; vertical-align:middle; }}
.res-tbl td:first-child {{ text-align:right; }}
.res-tbl td:nth-child(2) {{ text-align:right; {change_col_width} }}
...
<div style='overflow-x:auto; border:1px solid #e2e8f0; border-radius:12px; overflow:hidden'>
  <table class='res-tbl'>
    <thead>{header}</thead>
    <tbody>{body}</tbody>
  </table>
...
""", unsafe_allow_html=True)
    ...
    if not show_changes and not has_baseline:
        st.caption("💡 העלה דו\"ח מסלקה בטאב 'פורטפוליו נוכחי' כדי לראות השוואה למצב הקיים ועמודת שינויים נדרשים")

    st.markdown("---")
    voting_configured = hasattr(st, "secrets") and "gcp_service_account" in st.secrets
    st.markdown("**🗳️ איזו חלופה מעניינת אותך?**")
    st.caption("הבחירה שלך (אנונימית) עוזרת לנו להבין מה המשתמשים מעדיפים")

    voted_key = "voted_this_session"
    if st.session_state.get(voted_key):
        st.success(f"✅ תודה! בחרת: **{st.session_state[voted_key]}**")
    else:
        vote_cols = st.columns(4)
        vote_labels = [r["חלופה"] for r in rows]
        vote_icons  = ["⭐", "🎯", "📈", "🤝"]
        for col, label, icon, rrow in zip(vote_cols, vote_labels, vote_icons, rows):
            with col:
                if st.button(f"{icon} {label}", use_container_width=True, key=f"vote_{label}"):
                    tracks_str   = str(rrow.get("מסלולים",""))
                    managers_str = str(rrow.get("מנהלים",""))
                    if voting_configured:
                        ok = _write_vote(label, managers_str, tracks_str)
                        if ok:
                            _load_votes_cached.clear()
                    st.session_state[voted_key] = label
                    st.rerun()

    votes_label = "📊 סטטיסטיקת בחירות כל המשתמשים"
    if not voting_configured:
        votes_label += " (לא מופעל)"
    with st.expander(votes_label, expanded=False):
        if voting_configured:
            _render_votes_stats()
        else:
            st.info(
                "**הצבעות לא מופעלות עדיין.**\n\n"
                "כדי להפעיל: עקוב אחר ההוראות בקובץ `SETUP_VOTING.md` שבזיפ "
                "והגדר `[gcp_service_account]` ב-Streamlit Secrets."
            )
